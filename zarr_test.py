...
class ZarrHelper:
    ...
    def write_lv2(self, data, path, chunks=(720, 30)):
        ...
        if not isinstance(chunks, bool):
            with zarr.open(path, 'w') as zr:
                zr.create_dataset('timestamp', data=ts_list, shape=(
                    None, ), chunks=(chunks[0], ), dtype=np.int64)
                zr.create_dataset('value', data=value, shape=(
                    None, None), chunks=chunks, dtype=self._default_dtype, fillvalue=np.nan)
                zr.create_dataset('trim', data=trim, shape=(
                    None, ), chunks=(chunks[1], ))
        else:
            with zarr.open(path, 'w') as zr:
                zr.create_dataset('timestamp', data=ts_list, shape=(
                    None, ), chunks=chunks, dtype=np.int64)
                zr.create_dataset('value', data=value, shape=(
                    None, None), chunks=chunks, dtype=self._default_dtype, fillvalue=np.nan)
                zr.create_dataset('trim', data=trim,
                                  shape=(None, ), chunks=chunks)
        return True
    ...
    def append_lv2(self, data, path, chunks=(720, 30)):
        ...
        if (new_ts_shape > 0) & (new_tr_shape > 0):
            with zarr.open(path, 'a') as zr:
                ...
                value = data.reindex(new_ts_list, axis=0).reindex(
                    full_trim, axis=1).values
            ...
            with zarr.open(path, 'a') as zr:
                ...
                value = data.reindex(new_ts_list, axis=0).reindex(
                    old_trim, axis=1).values
                zr['value'].resize((full_ts_shape, old_trim.shape[0]))
                zr['value'][-new_ts_shape:] = value
        # Should not Happend
        elif new_tr_shape > 0:
            raise ValueError(
                'Datetime not updated but only trim updated, new trim: {}\npath: {}'.format(new_trim, path))
        return True
    ...
    def write_lv2_series(self, data, path, chunks=(720, 30)):
        ...
        if not isinstance(chunks, bool):
            with zarr.open(path, 'w') as zr:
                zr.create_dataset('timestamp', data=[ts], shape=(
                    None, ), chunks=(chunks[0], ), dtype=np.int64)
                zr.create_dataset('value', data=[value], shape=(
                    None, None), chunks=chunks, dtype=self._default_dtype, fillvalue=np.nan)
                zr.create_dataset('trim', data=trim, shape=(
            ...
            with zarr.open(path, 'w') as zr:
                zr.create_dataset('timestamp', data=[ts], shape=(
                    None, ), chunks=False, dtype=np.int64)
                zr.create_dataset('value', data=[value], shape=(
                    None, None), chunks=False, dtype=self._default_dtype, fillvalue=np.nan)
                zr.create_dataset('trim', data=trim,
                                  shape=(None, ), chunks=False)
    ...
    def read_lv2(self, path, start_ts, end_ts, appending_unit=60 * 60 * 24 * 5):
        ...
        for at in at_list:
            ...
            data = pd.concat([data, self._read_lv2(
                target_path, target_start, target_end)])
