var basket1, basket2, basket3, basket4, basket5; // 주식 종목들을 관리하는 Basket 객체
...
var valueWeightRatio1 = 0.5;    // 가치가중 50%
var valueWeightRatio2 = 0.75;   // 가치가중 75%
var valueWeightRatio3 = 1;      // 가치가중 100%
...
function initialize() {
    ...
    account1.accountName = "1. 절대가치 동일비중";
    basket1 = new Basket(account1, stock_num, IQEnvironment.aum * stock_weight);
    account2 = IQAccount.addAccount('0000-0000-02', '2. 절대가치 동일비중 거래대금필터', IQEnvironment.aum);
    basket2 = new Basket(account2, stock_num, IQEnvironment.aum * stock_weight);
    account3 = IQAccount.addAccount('0000-0000-03', '3. 절대가치 거래대금필터 가치가중 50%', IQEnvironment.aum);
    basket3 = new Basket(account3, stock_num, IQEnvironment.aum * stock_weight);
    account4 = IQAccount.addAccount('0000-0000-04', '4. 절대가치 거래대금필터 가치가중 75%', IQEnvironment.aum);
    basket4 = new Basket(account4, stock_num, IQEnvironment.aum * stock_weight);
    account5 = IQAccount.addAccount('0000-0000-05', '5. 절대가치 거래대금필터 가치가중 100%', IQEnvironment.aum);
...
//  Price OperatingIncome Increasing Ratio ( 전년동기대비 영업이익 증가액/시가총액 )
function pir(stock) {
...
function port_Value(universe, stock_number, filter_yn) {
    var UniverseFilter = universe.slice().filter(function (stock) {
        if (filter_yn === 'Y' && TradingValueFilter(stock) === 'N') { return false; }
        return true;
    });
...
function Port_Control(basket, account, universe, stockWeight, value_weight, pcrRatio, psrRatio, perRatio, porRatio, pgrRatio, bndRatio, rsiRatio) {
    var TOT_PSR_SCORE, TOT_PCR_SCORE, TOT_PER_SCORE, TOT_POR_SCORE, TOT_PGR_SCORE, TOT_BND_SCORE, TOT_RSI_SCORE;
    var PCR_SCORE = [];
    var PSR_SCORE = [];
    var PER_SCORE = [];
    var POR_SCORE = [];
    ...
    universe.forEach(function (stock) {
        ...
        var vgp = gp(stock);
        if (vgp <= 0) { vgp = 0; }
        PGR_SCORE[PGR_SCORE.length] = Math.pow(vgp, powerNumber);
    ...
    universe.forEach(function (stock) {
        ...
        var total_stock_budget = port_Budget * (
            ((PSR_SCORE[i] / TOT_PSR_SCORE) * psrRatio +
                ...
                (PGR_SCORE[i] / TOT_PGR_SCORE) * pgrRatio +
                (BND_SCORE[i] / TOT_BND_SCORE) * bndRatio +
                (RSI_SCORE[i] / TOT_RSI_SCORE) * rsiRatio
            ) / (pcrRatio + psrRatio + perRatio + porRatio + pgrRatio + bndRatio + rsiRatio)
        );
        ...
        if (stock_ratio > 15) { logger.debug('단일 종목의 비중이 15%를 넘음. 비중: ' + stock_ratio.toPrecision(4) + '%' + ' , 종목수: ' + universe.length + ' , 전략명: ' + account.accountName) }
...
function onDayClose(now) {
    if (IQDate.isRebalancingDay(now) || isFirst === true) {
        ...
        var port_value = port_Value(universe, stock_num, 'N');  //거래대금 1억 필터 없음
        var port_value_filter = port_Value(universe, stock_num, 'Y');  //거래대금 1억 필터 적용
        //주식비중,   가치가중비율,  pcrRatio, psrRatio, perRatio, porRatio, pgrRatio, bndRatio, rsiRatio
        Port_Control(basket1, account1, port_value, stock_weight, 0, 1, 1, 1, 1, 1, 1, 1);
        Port_Control(basket2, account2, port_value_filter, stock_weight, 0, 1, 1, 1, 1, 1, 1, 1);
        Port_Control(basket3, account3, port_value_filter, stock_weight, valueWeightRatio1, 1, 1, 1, 1, 1, 1, 1);
        Port_Control(basket4, account4, port_value_filter, stock_weight, valueWeightRatio2, 1, 1, 1, 1, 1, 1, 1);
        Port_Control(basket5, account5, port_value_filter, stock_weight, valueWeightRatio3, 1, 1, 1, 1, 1, 1, 1);
