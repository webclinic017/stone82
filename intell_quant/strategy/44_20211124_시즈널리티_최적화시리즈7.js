var basket1, basket2, basket3, basket4, basket5, basket6, basket7; // 주식 종목들을 관리하는 Basket 객체
var account1, account2, account3, account4, account5, account6, account7;
...
function initialize() {
    ...
    account2 = IQAccount.addAccount('0000-0000-02', '2. 모멘텀스코어', IQEnvironment.aum);
    basket2 = new Basket(account2, stock_num, IQEnvironment.aum * stock_weight);
    account3 = IQAccount.addAccount('0000-0000-03', '3. 모멘텀스코어+시즈널리티(중립)', IQEnvironment.aum);
    basket3 = new Basket(account3, stock_num, IQEnvironment.aum * stock_weight);
    account4 = IQAccount.addAccount('0000-0000-04', '4. 가속모멘텀', IQEnvironment.aum);
    basket4 = new Basket(account4, stock_num, IQEnvironment.aum * stock_weight);
    account5 = IQAccount.addAccount('0000-0000-05', '5. 가속모멘텀+시즈널리티(중립)', IQEnvironment.aum);
    basket5 = new Basket(account5, stock_num, IQEnvironment.aum * stock_weight);
    account6 = IQAccount.addAccount('0000-0000-06', '6. 절대모멘텀', IQEnvironment.aum);
    basket6 = new Basket(account6, stock_num, IQEnvironment.aum * stock_weight);
    account7 = IQAccount.addAccount('0000-0000-07', '7. 절대모멘텀+시즈널리티(중립)', IQEnvironment.aum);
    basket7 = new Basket(account7, stock_num, IQEnvironment.aum * stock_weight);
...
function Seasonality(MomentumWeight, NeutralityWeight) {
    var seasonRatio = MomentumWeight;
    var thisMonth = IQIndex.getIndex("004").getDate(0).getMonth() + 1;
    if (thisMonth >= 5 && thisMonth <= 10 && MomentumWeight > NeutralityWeight) { seasonRatio = NeutralityWeight; }
    return seasonRatio;
}
// 리밸런싱 수행 
function onDayClose(now) {
    if (IQDate.isRebalancingDay(now) || isFirst === true) {
        ...
        var MnSeasonWeight = Seasonality(MomentumWeight, 0.5);               //모멘텀 스코어에 시즈널리티(중립)를 적용한 주식비중
        var AcSeasonWeight = Seasonality(AccelMomWeight, 0.5);               //가속 모멘텀에 시즈널리티(중립)를 적용한 주식비중
        var AbSeasonWeight = Seasonality(AbsoluteWeight, 0.5);               //절대 모멘텀에 시즈널리티(중립)를 적용한 주식비중

        //주식비중,      가치가중비율,   pcrRatio, psrRatio, perRatio, porRatio, pirRatio, bndRatio, rsiRatio
        Port_Control(basket1, account1, port_value, stock_weight, valueWeightRatio1, 1, 1, 1, 1, 1, 1, 1);
        Port_Control(basket2, account2, port_value, MomentumWeight, valueWeightRatio2, 1, 1, 1, 1, 1, 1, 1);
        Port_Control(basket3, account3, port_value, MnSeasonWeight, valueWeightRatio2, 1, 1, 1, 1, 1, 1, 1);
        Port_Control(basket4, account4, port_value, AccelMomWeight, valueWeightRatio2, 1, 1, 1, 1, 1, 1, 1);
        Port_Control(basket5, account5, port_value, AcSeasonWeight, valueWeightRatio2, 1, 1, 1, 1, 1, 1, 1);
        Port_Control(basket6, account6, port_value, AbsoluteWeight, valueWeightRatio2, 1, 1, 1, 1, 1, 1, 1);
        Port_Control(basket7, account7, port_value, AbSeasonWeight, valueWeightRatio2, 1, 1, 1, 1, 1, 1, 1);
...
// 시뮬레이션 종료 함수
function onComplete() {
    logger.debug("계좌 총 평가금액은 " + IQUtil.getNumberWithCommas(Math.floor(account7.getTotalEquity())) + " 입니다.");
