...
var valueRatio = 0.2;           // 가장 저평가된 20%만 취한다 
var isFirst = true;             // 시뮬레이션 시작일에 바로 포트폴리오 신규 구성을 하기 위해 사용될 상태 변수
var valueWeightRatio = 1;       // 가치가중 100%
...
function initialize() {
    ...
    account1.accountName = "1. 절대가치 동일비중 자산배분";
    basket1 = new Basket(account1, stock_num, IQEnvironment.aum * stock_weight);
    account2 = IQAccount.addAccount('0000-0000-02', '2. 절대가치 가치가중 자산배분', IQEnvironment.aum);
...
function AssetAllocation(basket, account, stockWeight) {
    ...
    if (stockWeight < 0.95) {
        ...
        if (KR3Y_Tresaury == null) {
            ...
            basketEnter(basket, account, 'A305080', bond_ratio * 1);          //TIGER 미국채10년선물 
        }
...
function onDayClose(now) {
    if (IQDate.isRebalancingDay(now) || isFirst === true) {
        ...
        Port_Control(basket2, account2, port_value, stock_weight, valueWeightRatio, 1, 1, 1, 1, 1, 1, 1);
...
// 시뮬레이션 종료 함수
function onComplete() {
    logger.debug("계좌 총 평가금액은 " + IQUtil.getNumberWithCommas(Math.floor(account2.getTotalEquity())) + " 입니다.");
}